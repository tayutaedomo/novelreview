...
class NovelReviews {
  ...
  async restore(json_path) {
    // Read file
    const reviews_json = await fs.readFile(json_path, 'utf-8');
    if (! reviews_json) {
      throw new Error('JSON file is invalid.');
    }

    const restored = JSON.parse(reviews_json);

    // Restore instances
    this.reviews = restored.map(reviewData => {
      const review = new NovelReview();
...
class Novel {
  ...
  getLastUpdatedDate() {
    ...
    let s = this.lastUpdatedAt.replace('年', '-');
    s = s.replace('月', '-');
    s = s.replace('日 ', 'T');
    s = s.replace(' 更新', ':00+0900');

    // TODO: Needs zero padding
    //console.log(s);// TODO
